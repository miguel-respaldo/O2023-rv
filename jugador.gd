...
@onready var giro        := $izq_derecha
@onready var inclinacion := $izq_derecha/arriba_abajo
...
func _process(delta):
	...
	apply_central_force(giro.basis * input * 1200.0 * delta)
	
	if Input.is_action_just_pressed("ui_cancel"):
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		
	giro.rotate_y(izder_mov)
	inclinacion.rotate_x(arrab_mov)
	inclinacion.rotation.x = clamp(
